...
passDCall = Pass {pName = ["Direct call Conversion"], pFunc = runP }
    where runP ir@(IR tlf) = let r = IR (map (mexpr (getGlobals ir)) tlf) in (messageNoLn "Direct call Conversion" (disp r) Debug, Just r)
          mexpr g (TLFunction n cl p ex) = (TLFunction n cl p (expr g ex))


getGlobals (IR tlf) = map extractName tlf
